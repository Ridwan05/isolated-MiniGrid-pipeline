...
// ─── SUPABASE CONFIG ──────────────────────────────────────────────────────────
const SUPABASE_CONFIG = window.ISOLATED_MINIGRID_CONFIG || {};
...
const SEED_ISSUES = [
  { id: 1, project: "Bwari Rural Isolated Mini-Grid", category: "Commercial", description: "LOI counterparty delayed — AgAssetCo chasing", owner: "Emeka T.", raised: "2026-04-10", due: "2026-05-10", status: "Open", rag: "Amber" },
  { id: 2, project: "Kuje East Isolated Mini-Grid", category: "Financial Model", description: "Promise financial model v3 not yet received — blocks bankability submission", owner: "Fatima K.", raised: "2026-04-20", due: "2026-05-05", status: "Escalated", rag: "Red" },
  { id: 3, project: "Kwali Cluster Isolated Mini-Grid", category: "Deployment", description: "Daily deployment rate below 90/day — contractor capacity constraint", owner: "Amaka O.", raised: "2026-04-28", due: "2026-05-12", status: "Open", rag: "Amber" },
];
const SEED_DEPLOYMENT = [
  { id: 1, sitename: "Kwali North", project: "Kwali Cluster Isolated Mini-Grid", state: "FCT", LGA: "Kwali", connections: 140, PV: 42 },
  { id: 2, sitename: "Kwali South", project: "Kwali Cluster Isolated Mini-Grid", state: "FCT", LGA: "Gwagwalada", connections: 180, PV: 54 },
  { id: 3, sitename: "Bwari East", project: "Bwari Rural Isolated Mini-Grid", state: "FCT", LGA: "Bwari", connections: 80, PV: 24 },
];
    ...
    <div style={{ minHeight: "100vh", background: "#f2f4f7", fontFamily: "'Barlow','Segoe UI',sans-serif", color: "#1a1a2e" }}>
        ...
        <div className="rsp-header-inner" style={{ display: "flex", alignItems: "center", justifyContent: "space-between", paddingTop: 16, paddingBottom: 10 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            ...
            <div>
              ...
              <div style={{ fontSize: 18, fontWeight: 900, color: "#fff", letterSpacing: 0.3 }}>PROJECT PIPELINE MANAGER <span style={{ marginLeft: 10, fontSize: 10, background: "#3a9e5f", color: "#fff", padding: "3px 10px", borderRadius: 20, fontWeight: 700, verticalAlign: "middle", letterSpacing: 1 }}>ISOLATED MINI-GRID</span></div>
            </div>
          </div>
      ...
      <div className="rsp-main" style={{ padding: "24px 28px", maxWidth: 1300, margin: "0 auto" }}>
        ...
        {tab === "pipeline" && (<>
          ...
          <div style={{ marginBottom: 26 }}>
            <SectionHeader label="ISOLATED MINI-GRID BUSINESS MODEL" />
            <div className="rsp-biz-grid" style={{ display: "grid", gridTemplateColumns: "180px 1fr 1fr", gap: 14 }}>
              ...
              <div style={{ background: "#fff", borderRadius: 10, overflowX: "auto", boxShadow: "0 2px 8px rgba(0,0,0,0.07)" }}>
                <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
                  ...
                  <tbody>
                    {projects.map((p,i)=><tr key={p.id} style={{background:i%2===0?"#f7f9fc":"#fff",borderBottom:"1px solid #eef"}}><td style={{padding:"7px 10px",fontWeight:600,color:"#1a2a4a",fontSize:11}}>{p.name.replace(" Isolated Mini-Grid","")}</td><td style={{padding:"7px 10px",textAlign:"center"}}><Tick val={p.loi}/></td><td style={{padding:"7px 10px",textAlign:"center"}}><Tick val={p.jda}/></td><td style={{padding:"7px 10px",fontSize:11}}>{fmt(p.connections)}</td><td style={{padding:"7px 10px",fontWeight:700,fontSize:11}}>₦{p.size.toFixed(1)}Bn</td><td style={{padding:"7px 10px",color:"#3b6cb7",fontWeight:700,fontSize:11}}>{totalSize>0?((p.size/totalSize)*100).toFixed(1):0}%</td></tr>)}
        ...
        {/* ══ KPI DASHBOARD ══ */}
        {tab === "kpi" && (<>
          <SectionHeader label="KPI DASHBOARD — ISOLATED MINI-GRID" />
                ...
                <tbody>
                  {filteredDeployment.map((site,i)=>(
                    <tr key={site.id} style={{background:i%2===0?"#f7f9fc":"#fff",borderBottom:"1px solid #eef"}}>
                      ...
                      <td style={{padding:"9px 10px",color:"#555",fontSize:11}}>{site.project.replace(" Isolated Mini-Grid","")}</td>
      ...
      {/* MODALS */}
      {projectModal!==null&&<Modal title={projectModal==="add"?"Add Isolated Mini-Grid Project":"Edit Project"} onClose={()=>setProjectModal(null)} onSave={saveProject}><ProjectForm form={pForm} setForm={setPForm}/></Modal>}
